-- | Lecture 08: Software Transactional Memory (STM) and parallelism, built
-- around a tiny in-memory bank.
--
-- This lecture is a single self-contained module. The first half is the
-- /library/ — a tiny bank — and the second half is a narrated, runnable tour
-- ('main') that exercises it. Read the two halves alongside the output.
--
-- The bank teaches a handful of STM ideas:
--
...
--
-- The tour in 'main' is a sequence of "acts". Each act is a self-contained
-- mini-lesson you can talk through on its own:
--
...
--------------------------------------------------------------------------------
-- The library: a tiny bank on STM
--------------------------------------------------------------------------------
...
main = do
  ...
  putStrLn "============================================================"
  putStrLn "  Lecture 08 — a tour of STM and parallelism in Haskell"
...
act3_retryBlocks = do
  ...
  -- Spawn the waiting withdrawal in the background so main can deposit below.
  _ <- forkIO $ do
