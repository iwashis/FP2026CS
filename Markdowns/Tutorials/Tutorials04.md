...
1. **Maybe Monad — safe indexing and lookup chains**
   ...
   a. Implement `safeIndex :: [a] -> Int -> Maybe a` that safely returns the element at a given index (0-based), returning `Nothing` if the index is out of bounds. Do **not** use `(!!)`.
